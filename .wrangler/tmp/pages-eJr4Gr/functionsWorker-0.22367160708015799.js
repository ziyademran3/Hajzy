...
var paymobText = /* @__PURE__ */ __name((value) => {
  if (value === null || value === void 0) return "";
  if (typeof value === "boolean") return value ? "true" : "false";
  return String(value);
}, "paymobText");
var paymobTransactionHmacFields = /* @__PURE__ */ __name((transaction) => [
...
__name(paymobHmacIsValid, "paymobHmacIsValid");
...
async function createPaymobCheckout(env2, { amount, currency, title: title2, user, callbackBaseUrl }) {
  ...
  const intention = await paymobRequest("/v1/intention/", {
    ...
    redirection_url: env2.PAYMOB_RETURN_URL?.trim() || "com.hajzy.app://payment-result",
  ...
  if (env2.HAJZY_AUTH && (intention.intention_order_id || intention.id)) {
    await env2.HAJZY_AUTH.put(`payment:${intention.intention_order_id || intention.id}`, JSON.stringify({
      userId: user.id,
      reference,
      amountCents,
      currency: currency || "EGP",
...
async function onRequest(context2) {
  ...
  try {
    ...
    if (path === "/api/payments/paymob/webhook" && request.method === "POST") {
      ...
      const valid = await paymobHmacIsValid(transaction, receivedHmac, env2.PAYMOB_HMAC_SECRET?.trim());
      ...
      const rawPayment = paymentKey && env2.HAJZY_AUTH ? await env2.HAJZY_AUTH.get(paymentKey) : null;
      if (rawPayment && paymentKey) {
        const payment = JSON.parse(rawPayment);
        payment.status = transaction.success && !transaction.pending && !transaction.is_refunded && !transaction.is_voided ? "paid" : "failed";
        payment.transactionId = transaction.id;
        payment.paidAt = transaction.success ? (/* @__PURE__ */ new Date()).toISOString() : null;
        await env2.HAJZY_AUTH.put(paymentKey, JSON.stringify(payment), { expirationTtl: 60 * 60 * 24 * 30 });
      }
...
// ../.wrangler/tmp/pages-eJr4Gr/functionsRoutes-0.14122554500528217.mjs
var routes = [
  {
    routePath: "/api/:path*",
    mountPath: "/api",
